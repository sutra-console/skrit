...
- **[PROTOCOL.md](PROTOCOL.md)**: the full spec, covering COBS/CRC framing, message
...
DATA, 1 = CMD. **BLE** is dual-channel by GATT design: two skrit GATT services, DATA
(raw console; NUS-compatible UUID) and CMD (the frames). A device sets the `muxed`
...
`platforms/common` core already does all of this, so you write only a thin HAL.)
